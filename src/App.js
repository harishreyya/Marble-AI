...
const ChartPanel = ({ selectedMetrics, timePeriods }) => {
  ...
  return (
    <div>
      <LineChart width={600} height={300} data={filteredData} margin={{ top: 20, right: 30, left: 20, bottom: 5 }}>
        ...
        {selectedMetrics.map(metric => (
          <Line type="monotone" dataKey={metric} key={metric} stroke={`#${Math.floor(Math.random()*16777215).toString(16)}`} />
        ))}
      </LineChart>
    </div>
  );
...
const App = () => {
  ...
  const [timePeriods, setTimePeriods] = useState(['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'July']);
    ...
    <div>
      ...
      <div>
        ...
        <select
          ...
          onChange={e => handleMetricChange(Array.from(e.target.selectedOptions, option => option.value))}
        >
      ...
      <div>
        ...
        <select
          multiple
          value={timePeriods}
          onChange={e => handleTimePeriodChange(Array.from(e.target.selectedOptions, option => option.value))}
        >
          {data.map(entry => (
